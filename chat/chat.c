...
void *read_messages(void *arg) {   
    // 시그널 핸들러 함수에서 스레드 캔슬 발생시 지연 처리
    pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, NULL);   
    pthread_setcanceltype(PTHREAD_CANCEL_DEFERRED ,NULL); //  PTHREAD_CANCEL_ASYNCHRONOUS
    ...
    // 중복된 이름 있다면 종료
    for (int i = 0; i < 3; i++) 
        if(strcmp(cf->userlist[i], (char*)arg) == 0) {
            endwin(); 
            exit(0);
        }

    // 유저수가 3명인데 입장한 경우 unlink 후 종료
    if(cf->user_no > 2) {
        sem_unlink((char*)arg); 
        endwin(); 
        exit(0);
    ...
    while(1) {
        ...
        if(cf->message_index >= 10) {
            ...
            snprintf(cf->messages[9], sizeof(char) * 60, "[%s]: %s", (char *)arg, curmsg);    
        }   
        else {                  
            snprintf(cf->messages[cf->message_index], sizeof(char) * 60, "[%s]: %s", (char *)arg, curmsg);
            cf->message_index++;
        }
...
int main(int argc, char *argv[]) {  
    ...
    // 시그널 발생 후 스레드들이 종료되면 sem을 unlink
    sem_unlink(argv[1]);
    sem_close(sem);
